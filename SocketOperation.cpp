...
ssize_t SocketOperation::readv(int sockfd, struct iovec *iov, int iovcnt)
{
...
{
	vector<string> strs = split(addrIp, ':');
	uint16_t port;
...
int SocketOperation::shutdownWrite(int sockfd)
{
	...
	{
		LOG(ERROR) << "shutdown write  error";
	}
	return rst;
}
...
vector<string> SocketOperation::split(const string& str, char delim)
{
	vector<string> ret;
